...
>>>>>>> 27c8d4e40856daf243bf4c1f811004afd63b6139
...
>>>>>>> 27c8d4e40856daf243bf4c1f811004afd63b6139
  ...
  const handleApproveApplication = async (userId) => {
    try {
      ...
      if (response.success) {
        ...
        const approvedUser = pendingApplications.find(
  ...
  const handleUpdateReward = async (updatedRewardData) => {
    try {
      ...
      const response = await rewardsAPI.updateRewardStatus(
        updatedRewardData.id,
        {
      ...
      toast.success(
        `✅ Reward updated successfully for ${updatedRewardData.ambassadorName}!`
      )
    } catch (error) {
  ...
  const handleDeleteReward = async (reward) => {
    if (
      !window.confirm(
        `Are you sure you want to delete the reward of ${reward.currency} ${reward.amount} for ${reward.ambassadorName}?`
      )
    ) {
      return
    }
    ...
    try {
      ...
      const ambassadorHasOtherRewards = rewards.some(
        (r) => r.ambassadorId === reward.ambassadorId && r.id !== reward.id
      )
  ...
  const handleSubmitReward = async (rewardData) => {
    try {
      ...
      // Show success message
      toast.success(
        `✅ Reward of ${rewardData.currency} ${rewardData.amount} added successfully for ${rewardData.ambassadorName}!`
      )
    } catch (error) {
      console.error("Error submitting reward:", error)
      const errorMessage =
        error.response?.data?.message || error.message || "Failed to add reward"
      toast.error(`❌ Error: ${errorMessage}`)
      throw error
    } finally {
      setLoading(false)
  ...
  const handleDeleteAmbassador = async (ambassadorId) => {
    if (
      ...
      try {
        ...
        setAmbassadors((prev) =>
          prev.filter((ambassador) => ambassador._id !== ambassadorId)
        )
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const verifiedAmbassadors = allAmbassadors.filter(
        (user) => user.role === "ambassador" && user.isVerified === true
      )

      const pendingAmbassadors = allAmbassadors.filter(
        (user) => user.role === "ambassador" && user.isVerified === false
      )
